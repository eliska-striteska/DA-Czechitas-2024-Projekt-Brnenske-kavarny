...
# Načtení prvního datasetu (stažený podle Serach term: "kvárna")
with open(
...
cafes_cleaned = filter_keys_in_list_of_cafes(
    cafes,
    [
        "searchString",
        "title",
...
# Vytvoření prvního souboru s daty podle hesla "kavárna"
with open(
...
# Načtení druhého datasetu (stažený podle kategorie "cafe")
with open(
    "dataset_google-maps-extractor_2024-04-15_Brno_cafe.json", encoding="utf-8"
) as file:
    category_cafe = json.load(file)

category_cafe_cleaned = filter_keys_in_list_of_cafes(
    category_cafe,
    [
        "searchString",
        "title",
...
# Načtení druhého datasetu (stažený podle kategorie "coffee shop")
with open(
...
# Načtení třetího datasetu (stažený podle kategorie "espresso bar")
with open(
...
# Vytvoření 3 souborů s daty podle 3 vybraných kategorií (cafe, coffee shop. espresso bar)
with open(
...
# Porovnávání jmen kaváren (cafe["title"]) v jednotlivých datasetech:
...
with open(
    "maps_google_scraped_final_dataset.json", mode="w", encoding="utf-8"
) as output_file:
